...
#include "HHMI_Treadmill.h"

#define LOCTEXT_NAMESPACE "FHHMI_TreadmillModule"

void FHHMI_TreadmillModule::StartupModule()
{
...
void FHHMI_TreadmillModule::ShutdownModule()
{
    ...
    void run()
    {
        if ( _driver_file.good() )
        {
            ...
            for ( ;; )
            {
                // If compiled with optimization this gets removed and the
                // result is no motion
                if ( ( std::abs( _Vfwd ) < abs( dx[0] ) ) || ( std::abs( _Omega ) < abs( dx[1] ) ) )
                {
                    if ( _use_noise )
                    {
                        do
        ...
        for ( ;; )
        {
            ...
            // result is no motion
            if ( ( _driving_from_file &&
                    ( ( std::abs( _Vfwd ) < std::abs( dx[0] ) ) ||
                      ( std::abs( _Omega ) < std::abs( dx[1] ) ) ) ) ||
                    ( !_driving_from_file &&
                      ( ( std::abs( _Vss ) < std::abs( dy[0] ) ) ||
                        ( std::abs( _Vother ) < std::abs( dy[1] ) ) ) ) )
            {
                if ( _use_noise )
                {
                    do
...
IMPLEMENT_MODULE(FHHMI_TreadmillModule, HHMI_Treadmill)